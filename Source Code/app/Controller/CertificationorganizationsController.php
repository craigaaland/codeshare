 ...
 */
	...
	public $uses 		= array('Country','Member','ClubBranch','Club','ClubContact','CertificationOrganization','Certification','Degree');
		...
		{			
			...
			if(!empty($this->data)){				
				if( array_key_exists("keyword",$this->data) && !empty($this->data["keyword"]) && ($this->data["keyword"] != "Search by certification...") ) {					
					$conditions["OR"] = array(
			...
			if( !empty($this->params["named"]["keyword"]) ) {
				$conditions["OR"] = array(
									"Certification.branch_name LIKE" => "%".$this->params["named"]["keyword"]."%"
								);
				$keyword = $this->params["named"]["keyword"];
			}			
					
			
			$this->paginate = array("conditions"=>$conditions,'limit' => '10', 'order' => array('Certification.course' => 'ASC'));
