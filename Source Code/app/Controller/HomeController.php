 ...
 */
	...
	class HomeController extends AppController { 
		...
		$this->set("events",$this->Event->find('all',array('conditions'=>array('Event.trainer_id'=>$uid))));	       $tranrsdata=$this->CertificationTrainers->find('all',array('conditions'=>array('CertificationTrainers.trainer_id'=>$uid)));
...
$this->set("setSpecalistArr1",$setSpecalistArr1);
		...
		{	
			...
			if(trim( $_POST['evid'])!='')
			{	
				$scheduleCalendars=$this->ScheduleCalendar->find('first',array('conditions'=>array('ScheduleCalendar.id'=>$evid),'fields'=>array('ScheduleCalendar.mapwrkt','ScheduleCalendar.trainer_id','ScheduleCalendar.trainee_id','ScheduleCalendar.start','ScheduleCalendar.end')));
				...
				if(trim($_POST['newstartdate'])!='' && $ismapwrkt==1)
		        {
	...
	{	
...

		{
			...
			//$popupcondition=array('ScdetailPopup.trainer_id'=>$uid,'ScheduleCalendar.id'=>$evid);
		...
		//echo "<pre>"; print_r($popupschedetail); echo "</pre>"; die();
...
$this->set("setSpecalistArr1",$setSpecalistArr1);
    ...
    $setClientGoalArr3 = $this->Paginator->paginate('Goal');
...

			{
				...
				//$setClientGoalArr3=$this->Goal->find("all",array("conditions"=>array("Goal.trainee_id"=>$clientid,"Goal.status"=>1),'order' => array('Goal.start' => 'ASC'),'limit'=> $rec_limit,'offset'=>$offset));
...
$this->set("setSpecalistArr1",$setSpecalistArr1);
    ...
    'Subscription.id'=>$id)));
		...
		$this->set("events",$this->Event->find('all',array('conditions'=>array('Event.trainer_id'=>$trinaerid))));	       $tranrsdata=$this->CertificationTrainers->find('all',array('conditions'=>array('CertificationTrainers.trainer_id'=>$trinaerid)));
...

			{
				...
				if($dbusertype=='Trainer')

		   		{
		   			...
		   			$scheduleCalendars=$this->ScheduleCalendar->find('all',array('conditions'=>$sccondition,'fields'=>array('ScheduleCalendar.id','ScheduleCalendar.title','ScheduleCalendar.appointment_type','ScheduleCalendar.description','ScheduleCalendar.trainer_id','ScheduleCalendar.trainee_id','ScheduleCalendar.start','ScheduleCalendar.end'),"order"=>array("ScheduleCalendar.start"=>"ASC")));	
/*$log = $this->ScheduleCalendar->getDataSource()->getLog(false, false);
		debug($log);*/
		    	$this->set("calendarData",$scheduleCalendars);
   ...
   //$html2pdf->pdf->IncludeJS($script);
	...
	public function addclientingroup($gid=null)
     {
		...
		$client_data= $this->Trainee->find('list',array("conditions"=>array('Trainee.trainer_id'=>$setSpecalistArr['Trainer']['id']),'fields'=>array('Trainee.id','Trainee.full_name')));
	 ...
	 public function editclientingroup($gid=null)
     {
		...
		$sele_clients = $this->GroupMember->find('list',array("conditions"=>array('GroupMember.trainer_id'=>$setSpecalistArr['Trainer']['id']),'fields'=>array('GroupMember.client_id','GroupMember.client_name')));
	...
	public function groupdetail()
	{
		...
		'joins' => array(
					...
					'type' => 'LEFT',
					'conditions' => array(
					'GroupMember.client_id = session_purchase.client_id'
					)
					),
					array(
					'table' => 'workouts',
