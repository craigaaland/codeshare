...
function removePict(elem) {
		
	r = confirm("Are you sure want to remove the image ?");
	if(r){
		elem.innerHTML = "Please Wait,while deleting";
		$.ajax({
				url:site_url+"trainees/removePic/",
				type:"POST",
				data:{id:elem.id},
				success:function(e) {
					var response = eval(' ( '+e+' ) ');
...
function removePicne(elem) {
		
	r = confirm("Are you sure want to remove the image ?");
	if(r){
		elem.innerHTML = "Please Wait,while deleting";
		$.ajax({
				url:site_url+"news/removePic/",
				type:"POST",
				data:{id:elem.id},
				success:function(e) {
					var response = eval(' ( '+e+' ) ');
...
{	
	var foo = []; 
	$('#TraineeClubId :selected').each(function(i, selected){ 
	  foo[i] = $(selected).val(); 
	});	
	if(foo.length > 0){			
			$.ajax({
				type:"POST",
				url: site_url + 'trainees/listtrainers',
				data: "clubs="+foo,
				success:function(data){						
					$("#trainersID").html(data);					
				}
			});
		}	 
}
function getTrainerType()
{	
	var foo; 
	
	$('#TrainerTrainerType :selected').each(function(i, selected){ 
	  foo = $(selected).val(); 
	 if(foo=='I')
	 {
	 	//alert('k');
	 	$('#TrainerClubId option').removeAttr('selected');
	 	$('#TrainerClubId').prop('disabled', true);	
	 }
	 else
	 {
	 	$('#TrainerClubId').prop('disabled', false);	
	 }
	});	
