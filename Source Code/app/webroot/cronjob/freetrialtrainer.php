...
$sql = "SELECT id, first_name, last_name, email, added_date, trainer_type, subscriptionplan, phone FROM trainers";
...
if (mysql_num_rows($result) > 0) {
	...
	if ($actual_diff == 5 && $row["trainer_type"] == I && ($row["subscriptionplan"] == '' || $row["subscriptionplan"] == NULL))
		{
	...
	elseif ($actual_diff >= -15 && $actual_diff <= -1 && $row["trainer_type"] == I && ($row["subscriptionplan"] == '' || $row["subscriptionplan"] == NULL))
	{
		$email_dea_left = $row["email"];
		$sql_deactivate_left = "UPDATE `trainers` SET `after_sub_trial_end`= '1' WHERE `email`= '$email_dea_left'";
		$result_deactivate_left = mysql_query($sql_deactivate_left) or die("Query Error ".mysql_error());
	}
	elseif ($actual_diff == -15 && $row["trainer_type"] == I && ($row["subscriptionplan"] == '' || $row["subscriptionplan"] == NULL))
	{	
		$email_dea_left = $row["email"];
		$sql_deactivate_left = "UPDATE `trainers` SET `after_sub_trial_end`= '1' WHERE `email`= '$email_dea_left'";
		$result_deactivate_left = mysql_query($sql_deactivate_left) or die("Query Error ".mysql_error());
