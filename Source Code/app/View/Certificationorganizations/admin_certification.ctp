...
<?php echo $this->Form->create(null,array('controller'=>'certificationorganizations', 'action'=>'certification', 'enctype'=>'multipart/form-data', 'class'=>'mainForm', 'id'=>'valid')); ?>
...
<div class="table">
    <div class="head">
		<h5 class="iFrames">Manage Certifications</h5>
		<div class="rowElem noborder" style="clear:none;margin-top: -4px;">
			<?php if(isset($certifications) && count($certifications) > 0){ ?>
				<?php  echo $this->Form->select('Certification.statusTop',unserialize($config['status_array']),array('empty'=>'Select','class'=>'topAction','style'=>'width:20%')); ?>&nbsp;
				<?php  echo $this->Form->submit('OK', array('style'=>'float:none;margin-left: -1px;','class'=>'blueBtn submitForm','name'=>'data[Certification][submit]', 'div'=>false, 'onclick'=>"return del('data[Certification][id][]')",'id'=>'StatusTop'));?>				
				
			<?php 	} ?> 		
				<a href="<?php echo $this->Html->url(array('controller'=>'certificationorganizations', 'action'=>'addcertification')); ?>" style="float: right; margin-top:0px; padding: 2px 13px;margin-right:0px;" class='blueBtn'>Add New</a>
		</div>		
	</div>
        <div class="dataTables_wrapper" id="example_wrapper">
			<?php echo $this->Form->create('Certification',array('controller'=>'certificationorganizations','action'=>'certification/'.$this->params["pass"][0],'class'=>'mainForm')); ?>	
				<div class="">
					<div class="dataTables_filter" id="example_filter" style="right:15%;top:-38px;">
						<label>Search: <input type="text" name="keyword" value="<?php echo $keyword;?>" placeholder="Search by certification..."/>
			...
			<?php 
				...
				if(isset($certifications) && count($certifications) > 0){
					$this->Paginator->options(array('url' => array("controller"=>"certificationorganizations","action"=>"certification/$tab/keyword:$keyword")));
			...
			<table cellspacing="0" cellpadding="0" border="0" id="" class="display">
                <thead>
                    <tr>
						<th class="ui-state-default" rowspan="1" colspan="1" width="3%">
							<div style="padding-right:8px;"><?php echo $this->Form->text('Certification.all', array('type'=>'checkbox', 'id'=>'data[Certification][all]', 'onclick'=>"selectDeselect('data[Certification][id][]', this.name);"));?></div>
						</th>
						<th class="ui-state-default" rowspan="1" colspan="1" width="3%">
				...
				if(isset($certifications) && count($certifications) > 0){
					...
					if(trim($prow['Certification']['full_name']))
						$full = "(".trim($prow['Certification']['full_name']).")";
					else
						$full = "";	
				?>
				...
				<tr class="gradeA <?php if($count%2==0) echo 'odd'; else echo 'even';?>">
					<td align="center"><?php echo $this->Form->text('Certification.id][', array('type'=>'checkbox', 'value'=>$prow['Certification']['id'],  'onclick'=>"checkSelection('data[Certification][id][]', 'data[Certification][all]')"));?></td>
					<td align="center"><?php echo $index;?></td>
					<td><a href="<?php echo $config['url']; ?>admin/certificationorganizations/viewcertification/<?php echo $prow['Certification']['id']; ?>"  ><?php echo $prow['Certification']['name']."<br>".$full; ?></a></td>
					<td align="center"><?php echo $prow['CertificationOrganization']['name'];?></td>
						
										
					<td align="center"><?php echo date($config['date_format'], strtotime($prow['Certification']['added_date'])); ?></td>
					<td align="center"><?php 
						if($prow['Certification']['status']==1){
					...
					<td align="center"><?php echo $this->Html->link($this->Html->image('edit-icon.png'),array('controller'=>'certificationorganizations','action'=>'editcertification/'.$prow['Certification']['id'] ), array('title'=>'Edit Certification','escape'=>false));?>
					</td>
