    ...
    <div class="row">
     <?php echo $this->element('lefttrainer');?>
      <div class="eight inside-head columns">
        ...
        <div>
        <h2>Edit Clients in Group</h2>
        <form accept-charset="utf-8" method="post" id="validaddgroup" class="resform-wrap" enctype="multipart/form-data" controller="home" action="/home/editclientingroup/">
		...
		<div class="twelve columns">   
			<?php
			$selector=0;
			foreach ($sele_clients as $ky=>$myikeys){
			$sel[]=$ky;
			}		
			?>
			<select id="GroupMemberGroupClients" multiple="multiple" required="required" class="sltbx" name="data[GroupMember][group_clients][]">
			<option value="">-- Select Client --</option>
			<?php foreach ($client_data as $key=>$clb) {?>
			<?php 
			if(in_array($key,$sel)){
			$selector=1;
			}
