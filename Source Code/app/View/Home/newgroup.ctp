    ...
    <div class="row">
     <?php echo $this->element('lefttrainer');?>
      <div class="eight inside-head columns">
        ...
        <div>
        <h2>Add New Group</h2>
        <form accept-charset="utf-8" method="post" id="validaddgroup" class="resform-wrap" enctype="multipart/form-data" controller="home" action="/home/newgroup/">
		   
       
        <input type="hidden" id="GroupTrainerId" value=" <?php echo $setSpecalistArr[$utype]['id'];?>"  name="data[Group][trainer_id]"/>
        
		<div class="twelve columns">
		
        <?php echo $this->Form->text('Group.group_name', array('maxlength'=>255,'id'=>'group_name','class'=>'validate[required]','placeholder'=>'Group Name')); ?>
        <?php echo $this->Form->error('Group.group_name', null, array('class' => 'error')); ?>
		</div>
				
		<div class="twelve form-select columns">
        <select id="groupstatus" onchange="document.getElementById('customSelectStatus').value= this.options[this.selectedIndex].text" class="topAction" name="data[Group][status]">
            ...
            <input type="submit" class="submit-nav" name="submit" onclick="return addgroupconfirm();" value="Save"  />
          </form>
        </div>
